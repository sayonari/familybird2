...
| 音楽 | NSD.Lib (2曲+SE) | 自作5chドライバ「SAYODRV」+ DPCM打楽器, 全7曲 |
| ステージBGM | 1曲 | 10曲から毎回ランダム (オリジナル6 + PD名曲アレンジ4) + スター専用曲 |
| ハイスコア | なし | タイトル画面に表示(電源断まで保持) |
| 演出 | - | 昼→夕→夜のパレット変化, 羽ばたき星エフェクト, 金色スコア, メダル/NEW RECORD |
| コンボ | - | コイン連続取得で価値上昇 (5→10→15→20点) + 得点ポップ表示 |
| ランク | - | 画面左上に常時ランク+進捗ゲージ (ランク99まで). ゲームオーバーに32種の称号 |
| ボス土管 | - | スコア15以降まれに出現する金色の狭ゲート土管 (+3点) |
| 風 | - | 上昇/下降気流 (葉っぱと風の粒で可視化) |
| 実績 | - | 8種のAWARDS. リセット耐性 + ブラウザ版はlocalStorage自動セーブ |
| 難易度 | 一定 | スコア30/70でスクロール加速, キャラごとに飛行性能が異なる |
| ポーズ | なし | STARTでポーズ |
...
- タイトル: アニメOP風ポップス (王道進行サビ)
- ステージ1: 熱血ロック (ビブラートリード/オクターブベース/DPCMドラム)
- ステージ2: キラキラアイドル風 (16分アルペジオ)
- ステージ3: かわいいポップ (スタッカート)
- ステージ4: 勇者マーチ風 (付点リズム+タム行進)
- ステージ5: 夜ふかしスウィング風 (シャッフル+ウォーキングベース)
- ステージ6: トルコ行進曲 (モーツァルト, PD) アレンジ
- ステージ7: エンターテイナー (ジョプリン, PD) アレンジ
- ステージ8: 天国と地獄 (オッフェンバック, PD) アレンジ
- ステージ9: コロブチカ (ロシア民謡, PD) アレンジ
- ステージ10: 技巧デモ曲 (高速アルペジオ疑似和音/32分ラン, オリジナル)
- スター無敵: 225BPM全力疾走ループ
- ゲームオーバー: 下降ジングル / スタッフロール: ワルツ

コード進行や様式はアニソン・ゲーム音楽の定番を使いつつ，メロディはすべてオリジナル．
（PD曲は著作権消滅済みの古典を自作アレンジ）
...
| 機能 | MML | 内容 |
|---|---|---|
| ゲートタイム | `q1`〜`q8` | 音符長に対する発音比率．`q8`=レガート，`q2`=スタッカート |
| 音量アクセント | `v0`〜`v15` | 音符ごとの強弱 |
| デューティ変調 | `y0`〜`y3` | 矩形波の音色を曲中で切り替え |
| ピッチエンベロープ | 楽器定義 | しゃくり上げ・ドロップ・オクターブ突入 |
| 遅延ビブラート | 楽器定義 | 伸ばした音の後半だけ揺らす |
| 高速アルペジオ | 楽器定義 | 1フレームごとに和音構成音を巡回する疑似和音 |
| 音量エンベロープ | 楽器定義 | 任意長，終端ホールド |
